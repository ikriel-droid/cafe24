...
def test_claim_note_is_recorded_in_audit_log(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        response = client.post(
            "/api/claims/1/notes",
            json={"note": "고객이 사진 재전송 예정", "actor": "qa_operator"},
        )
        assert response.status_code == 200
        payload = response.json()
        ...
        assert latest_log["payload_json"]["note"] == "고객이 사진 재전송 예정"
...
def test_dashboard_summary_includes_cafe24_overview(monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        client.post("/api/integrations/cafe24/mock-sync")
        refreshed = client.get("/api/dashboard/summary")
        assert refreshed.status_code == 200
        payload = refreshed.json()
        assert payload["cafe24"]["last_sync_result"] == "mock_completed"
        assert payload["cafe24"]["health_status"] == "setup_needed"
        assert payload["cafe24"]["next_action_href"] == "/integrations/cafe24"
        assert payload["cafe24"]["latest_event_title"] == "Cafe24 Mock Sync completed"
        assert payload["cafe24"]["latest_event_status"] == "success"
